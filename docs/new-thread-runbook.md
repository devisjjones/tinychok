...
- первую строку по новой задаче всегда начинать с явной валидации понимания:
  - `Беру задачу ...`
  - `Понял так: ...`
...
Если во время задачи происходит рефакторинг и логика уезжает из одного файла в отдельные helper / hook / persistence-модули:

- нельзя оставлять docs и contract tests с прежним source-of-truth
- в той же задаче нужно обновить:
  - handoff docs
  - release contracts
  - gate / ui-contract tests, если они ссылались на старую географию кода
- особенно это касается выноса логики из `src/App.tsx`: следующий тред не должен снова искать runtime-поведение только там
...
- первая строка итога в формате `Задача ... выполнена так-то.`
- что именно изменено
- какие файлы были затронуты
- какие автотесты прошли
- в каждом промежуточном статусе и в финальном ответе должны быть явные строки:
  - `Автотесты: ...`
  - `Новый автотест: да/нет`
