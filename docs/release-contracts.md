...
### 11.1.2. Group Left Rail Card Contract

- `group` cards в левом списке используют отдельный compact layout, а не общий default `chat-card`
- вертикальный padding должен оставаться компактным, без лишнего воздуха над верхней строкой и под preview
- group avatar в левом списке остаётся увеличенным относительно default avatar и слегка сдвинутым влево, чтобы не оставлять пустую рамку вокруг изображения
