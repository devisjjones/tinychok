...
test('direct, group and thread feeds keep compact bubble spacing while channel posts stay roomier', () => {
  ...
  assert.match(
    appCss,
    /\.group-room-feed \.bubble:not\(\.media-only-bubble\):not\(\.emoji-only-message\),\s*\.room-thread-feed \.bubble:not\(\.media-only-bubble\):not\(\.emoji-only-message\) \{\s*padding-top: 10px;\s*padding-bottom: 8px;/u,
  )
  assert.match(
    appCss,
...
test('mobile auth keyboard flow keeps the submit action visible above the on-screen keyboard', () => {
  ...
  assert.match(authViewportSource, /resolveMobileViewportRevealDelta/u)
  assert.match(authViewportSource, /resolveMobileViewportTopHideDelta/u)
  assert.match(authScreenSource, /resolveMobileViewportKeyboardInset/u)
  assert.match(authScreenSource, /const authCardBrandRef = useRef<HTMLDivElement \| null>\(null\)/u)
  assert.match(authScreenSource, /const viewportBaselineHeightRef = useRef\(0\)/u)
  assert.match(authScreenSource, /viewportBaselineHeightRef\.current = Math\.max\(window\.innerHeight, window\.visualViewport\?\.height \?\? 0\)/u)
  assert.match(authScreenSource, /layoutViewportHeight:\s*viewportBaselineHeightRef\.current/u)
  assert.match(authScreenSource, /const currentViewportHeight = window\.visualViewport\?\.height \?\? window\.innerHeight/u)
  assert.match(authScreenSource, /const currentViewportOffsetTop = window\.visualViewport\?\.offsetTop \?\? 0/u)
  assert.match(authScreenSource, /shell\.dataset\.keyboardOpen = 'true'/u)
  assert.match(authScreenSource, /resolveMobileViewportTopHideDelta\(/u)
  assert.match(authScreenSource, /elementBottom:\s*authCardBrand\.getBoundingClientRect\(\)\.bottom/u)
  assert.match(authScreenSource, /elementBottom:\s*submitButton\.getBoundingClientRect\(\)\.bottom/u)
  assert.match(authScreenSource, /shell\.scrollTop \+= nextScrollDelta/u)
  assert.match(authScreenSource, /settleTimeout = window\.setTimeout\(/u)
  assert.match(authScreenSource, /ref=\{authCardBrandRef\}/u)
...
test('video-note bubbles play inline inside the circle instead of opening the global media viewer overlay', () => {
  ...
  assert.match(
    bubbleSource,
    /useEffect\(\(\) => \{[\s\S]*if \(!isVideoNote \|\| !isInlinePlaying \|\| typeof window === 'undefined'\) \{[\s\S]*const syncPlaybackProgress = \(\) => \{[\s\S]*normalizeInlinePlaybackProgress\(inlineVideo\.currentTime,\s*inlineVideo\.duration\)[\s\S]*window\.requestAnimationFrame\(syncPlaybackProgress\)/u,
  )
  assert.match(
    bubbleSource,
    /const videoNotePlaybackPercent = Math\.min\([\s\S]*isVideoNotePlaying \? Math\.max\(0\.8,\s*videoNotePlaybackProgress \* 100\) : 0[\s\S]*isVideoNotePlaying \? \([\s\S]*className="bubble-attachment-video-note-progress"[\s\S]*className="bubble-attachment-video-note-progress-fill"[\s\S]*minWidth:\s*isVideoNotePlaying \? '2px' : undefined,[\s\S]*width: `\$\{videoNotePlaybackPercent\}%`/u,
  )
  assert.match(
    bubbleSource,
  ...
  assert.match(
    appCss,
    /\.bubble-attachment-video-note-progress\s*\{[\s\S]*position:\s*relative;[\s\S]*justify-self:\s*stretch;[\s\S]*width:\s*calc\(100% - 24px\);[\s\S]*margin:\s*2px 12px 0;[\s\S]*height:\s*3px;[\s\S]*background:\s*rgba\(95,\s*66,\s*46,\s*0\.14\);[\s\S]*overflow:\s*hidden;/u,
  )
  assert.match(
    appCss,
