...
test('quiet settings scene keeps category-specific notification contract wired through app, shared types and docs', () => {
  ...
  assert.match(appSource, /Уведомления тредов/u)
...
test('narrow mobile view keeps settings, room headers and admin panels from overflowing', () => {
  ...
  assert.match(appCss, /@media \(max-width: 960px\) \{[\s\S]*\.room-header\s*\{[\s\S]*grid-template-columns:\s*34px minmax\(0,\s*1fr\) auto;[\s\S]*gap:\s*8px;/u)
  assert.match(appCss, /@media \(max-width: 960px\) \{[\s\S]*\.room-mobile-back\s*\{[\s\S]*width:\s*34px;[\s\S]*height:\s*54px;[\s\S]*border-radius:\s*16px;[\s\S]*box-shadow:\s*0 10px 24px rgba\(17,\s*18,\s*20,\s*0\.08\);/u)
  assert.match(appCss, /@media \(max-width: 960px\) \{[\s\S]*\.room-menu-button,\s*[\s\S]*\.room-star,\s*[\s\S]*\.room-group-actions-toggle\s*\{[\s\S]*width:\s*42px;[\s\S]*height:\s*54px;[\s\S]*border-radius:\s*16px;/u)
  ...
  assert.match(appCss, /@media \(max-width: 420px\) \{[\s\S]*\.room-header\s*\{[\s\S]*padding:\s*10px 10px 9px;[\s\S]*grid-template-columns:\s*30px minmax\(0,\s*1fr\) auto;/u)
  assert.match(appCss, /@media \(max-width: 420px\) \{[\s\S]*\.room-id \.avatar\.large\s*\{[\s\S]*width:\s*54px;/u)
  assert.match(appCss, /@media \(max-width: 420px\) \{[\s\S]*\.room-mobile-back\s*\{[\s\S]*width:\s*30px;[\s\S]*height:\s*50px;[\s\S]*border-radius:\s*15px;/u)
  assert.match(appCss, /@media \(max-width: 420px\) \{[\s\S]*\.room-menu-button,\s*[\s\S]*\.room-star,\s*[\s\S]*\.room-group-actions-toggle\s*\{[\s\S]*width:\s*38px;[\s\S]*height:\s*50px;[\s\S]*border-radius:\s*15px;/u)
...
test('thread admin archive flow is wired through admin ui, route and store contracts', () => {
  ...
  assert.match(adminAppSource, /Архивированные треды/u)
  assert.match(adminAppSource, /Активные треды/u)
  assert.match(adminAppSource, /Архивировать тред/u)
  assert.match(adminAppSource, /Разархивировать тред/u)
  assert.match(adminAppSource, /admin-button-with-icon-archive/u)
  ...
  assert.match(storeSource, /Тред находится в архиве и недоступен пользователям/u)
...
test('mobile direct-room status clamps to two lines with explicit expand and collapse toggle', () => {
  const repoRoot = fileURLToPath(new URL('../..', import.meta.url))
  const directRoomSource = readFileSync(join(repoRoot, 'src', 'rooms', 'DirectChatRoom.tsx'), 'utf8')
  const appCss = readFileSync(join(repoRoot, 'src', 'App.css'), 'utf8')
  const rolloutDoc = readFileSync(join(repoRoot, 'docs', 'staging-rollout-status.md'), 'utf8')
