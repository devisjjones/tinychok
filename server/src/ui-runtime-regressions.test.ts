...
test('attachment storage cleanup copy stays wired into preview and bubble rendering', () => {
  ...
  assert.match(appSource, /У вас закончилось место в хранилище\./u)
  assert.match(appSource, /Старые отправленные фото и файлы будут удалены автоматически/u)
  assert.match(appSource, /composer-attachment-inline-premium/u)
  assert.match(appSource, /composer-attachment-premium-upsell-copy/u)
  assert.match(appSource, /composer-attachment-premium-crown-brown/u)
  assert.match(appSource, /Премиум<\/span>/u)
  assert.match(appSource, /<span>Премиум<\/span>[\s\S]*composer-attachment-premium-crown-brown/u)
...
test('owner-only admin storage exports stay wired to password-confirmed ZIP download and archive controls', () => {
  ...
  assert.match(storeSource, /retention-only direct attachments that are intentionally hidden from user storage UI/u)
...
test('composers stay on plain textarea inputs without formatting toolbar', () => {
  ...
  assert.match(groupRoomSource, /resizeComposerTextarea\(textarea\)/u)
  assert.match(channelRoomSource, /resizeComposerTextarea\(textarea\)/u)
...
test('narrow mobile view keeps settings, room headers and admin panels from overflowing', () => {
  ...
  assert.match(appCss, /@media \(max-width: 420px\) \{[\s\S]*\.room-header\s*\{[\s\S]*padding:\s*11px 12px 10px;/u)
